...
const AI_API_KEY = process.env.OPENROUTER_API_KEY; // Use environment variable!
...
const FALLBACK_MODELS = [
  ...
  'moonshotai/kimi-k2:free' // Keep as last option
];
...
export const getAIRecommendations = async (query: string): Promise<AIRecommendation> => {
  for (const model of FALLBACK_MODELS) {
    try {
      ...
      const response = await axios.post(
        AI_API_BASE_URL,
        {
          ...
          messages: [
            {
              role: 'system',
              content: 'You are a compassionate and knowledgeable librarian assistant with deep understanding of literature and human emotions. Given a user\'s search query, you should: 1. ALWAYS provide meaningful book recommendations, especially for emotional or descriptive queries 2. If the user expresses feelings (sad, happy, confused, lost, etc.), recommend books that address those emotions or provide comfort 3. For vague or emotional queries, suggest specific book titles and authors that match the mood or situation 4. Enhance the search query to find relevant books in digital libraries 5. Provide alternative search terms for better results. IMPORTANT: Even if the query is vague like "feeling sad", "don\'t know what to read", "need inspiration", you MUST provide specific book recommendations with titles and authors. For emotional queries, consider recommending: Self-help and personal development books, Fiction that deals with similar emotions, Inspirational memoirs and biographies, Philosophy and wisdom literature, Poetry collections, Classic literature that explores human condition. Respond ONLY with valid JSON (no markdown code blocks) in this exact format: {"enhancedQuery": "improved search query for digital libraries", "recommendations": ["Specific Book Title by Author Name", "Another Book Title by Author Name", "Third Book Title by Author Name"], "searchTerms": ["alternative search term 1", "alternative search term 2", "alternative search term 3"]}'
            },
            {
              role: 'user',
      ...
      console.error(`AI recommendation error with ${model}:`, error.response?.data || error.message);
      continue; // Try next model
    }
  }
  ...
  return {
    ...
    searchTerms: [query, "inspirational books", "self help", "personal development"]
  };
};        {
          model: model,
          messages: [
